...
<<<<<<< HEAD
			...
			for (var i = 0 ; i < res.length ; i++) {
			    console.log(res[i]);
			    //max of 50 reads from res...
			    var jname = res[i].jobname;
			    jname = jname.replace("-ANALYSE","").replace("-REF","").replace("-XAS","");
			    if(!properName(jname)) continue;
			    if(occurs[jname]) occurs[jname]++; else occurs[jname]=1;
			    //hours is incorrect, need to add up from all 3 jobs
			    if(occurs[jname] < 3) continue;
			    //sort by jobname, buttons to change sorting type (recursing inputs)
			    //(Today) (Day) (Month) (Year) (Search by Molname) (show X results)
			    myText += "<tr class='listitem'>";
			    myText += "<td width=55\%>" + jname + "</td><td class=\"statusnone\">" + "x" //res[i].rawhours
...
function individualJobOutput(jobName, machine) {
     ...
     var myHtml = "<table width=100\%><th align=left>Results for " + jobName + "</th></table><br>";
      myHtml += "<table width=100\%><tr><td width=55\%><div id=\"flotwrapper\">Title?<br><div id=\"placeholder\"></div></div></td><td width =5\%></td><td width=40\%>";

      //Creates the jmol applet, couldn't use jmolapplet (wipes html)
     var jMolOnLoadScript = "javascript drawMol('results');";
     myHtml += '<object id="jmolAppletresults" width="250" height="250" type="application/x-java-applet" name="jmolAppletresults"><param value="9670009897246854" name="syncId"><param value="true" name="progressbar"><param value="blue" name="progresscolor"><param value="black" name="boxbgcolor"><param value="white" name="boxfgcolor"><param value="Downloading JmolApplet ..." name="boxmessage"><param value="JmolAppletSigned.jar" name="archive"><param value="true" name="mayscript"><param value="javascript/jmol-12.2.25" name="codebase"><param value="JmolApplet" name="code"><param value="-Xmx512m" name="java_arguments"><param value="'+jMolOnLoadScript+'" name="script"><p style="background-color:yellow; color:black; width:250px;height:250px;text-align:center;vertical-align:middle;">You do not have Java applets enabled in your web browser, or your browser is blocking this applet.<br>Check the warning message from your browser and/or enable Java applets in<br>your web browser preferences, or install the Java Runtime Environment from<a href="http://www.java.com">www.java.com</a><br></p>';

     myHtml += "</td></tr></table>";  
     console.log(myHtml);
     //add Flot
     //Postprocessing
     $('#individualjob').html(myHtml);
     $('#individualjob').trigger('create');
     $(document).ready(makePlot());   
}
function makePlot() {
    var d1 = [];
...
<<<<<<< HEAD
    ...
    //post webdata
    $.newt_ajax({type: "PUT", 
		url: "/file/"+machine+"/global/scratch/sd/"+myUsername+"/"+materialName+"/"+ "webdata.in",
		data: webdata,
		success: function(res) {;},});
